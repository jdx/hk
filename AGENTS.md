...
generated by Communique. Write them for a hk user who has not read the diff
...
For a hypothetical fix, prefer `fix(check): lint staged files with spaces in their names`
over `fix: address review feedback`. Its description should show the affected filename and check command, then explain which files are now checked.
