...
hk is configured via `hk.pkl` which is written in [pkl-lang](https://pkl-lang.org/) from Apple. hk evaluates it with the built-in [pklr](https://github.com/jdx/pklr) evaluator, so the pkl CLI is not required.
