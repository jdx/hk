...
//
// Both faces are bundled (docs/.vitepress/fonts) and loaded with FontFace
// before the first frame: Space Grotesk for everything set in type, and
// Liberation Mono, the face the site's own mono stack resolves to on CI, for
// terminals, code and hashes. Bundling it makes a local render match CI's.
...
/** Registered at weights 400 and 700 only; heavier requests get the bold. */
export const MONO = '"Liberation Mono", monospace';

export const font = (size: number, weight = 600, family = DISPLAY): string =>
  `${weight} ${size}px ${family}`;

// bible.ts PALETTE.paper and PALETTE.warm. The palette's module imports this
// one's types, so the two colours are repeated here; a test keeps them equal.
/** Captions and copy: warm paper. */
export const PAPER = "#f4eee3";
/** Code in copy: hk's warm keyword colour. */
export const CODE = "#eac18e";
...
// mono and warm, a little smaller so its x-height matches.
