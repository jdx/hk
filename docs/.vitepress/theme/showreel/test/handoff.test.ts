...
// bar line, holds except the whip, each scene starting from the one it
// inherits and ending on the one it owes, and the numbers for the frames the
// kit has no preset for. handoff-frames.test.ts draws the frames and holds
// every scene to them.
...
test("each scene returns its handoffs' lit screen on its side of the bar line, whatever the facts", () => {
  ...
  for (const variant of ["both", "one", "none"] as const) {
    ...
    for (const id of BOUNDARIES) {
      ...
      assert.deepEqual(norm(out.lit?.(out.end - out.start - 1 / 120, env)), want, `${variant}: ${h.from}'s last frame is lit as ${id} is`);
      assert.deepEqual(norm(scene(h.to).lit?.(0, env)), want, `${variant}: ${h.to}'s first frame is lit as ${id} is`);
    }
  }
