...
fn run_pklr<T: DeserializeOwned>(path: &Path) -> Result<T> {
    ...
    .map_err(|e| eyre::eyre!("{e}"))?;
    let json = pklr_normalize_output(json);
...
/// normalizations that hk's Config.pkl applies are missing. This function
/// replicates them: _type tags for Step/Group/Regex, string-to-array
/// coercion for depends/stage, and bool-to-string for stash.
fn pklr_normalize_output(mut value: serde_json::Value) -> serde_json::Value {
        ...
        for hook in hooks.values_mut() {
                ...
                .and_then(|s| s.as_object_mut())
            {
                for step in steps.values_mut() {
                    pklr_normalize_step_or_group(step);
                }
            }
        }
    }
...
fn pklr_normalize_step_or_group(value: &mut serde_json::Value) {
    let Some(obj) = value.as_object_mut() else {
        return;
    };
    if !obj.contains_key("_type") {
        if obj.contains_key("steps") {
            obj.insert("_type".into(), "group".into());
            ...
            obj.insert("_type".into(), "step".into());
        }
    }
    // Coerce string → [string] for depends and stage
    for key in &["depends", "stage"] {
        if let Some(serde_json::Value::String(s)) = obj.get(*key) {
            let arr = serde_json::Value::Array(vec![s.clone().into()]);
            obj.insert((*key).to_string(), arr);
        }
    }
    // Coerce bool → string for stash
    if let Some(serde_json::Value::Bool(b)) = obj.get("stash") {
        let s = if *b { "git" } else { "none" };
        obj.insert("stash".into(), s.into());
    }
    // Tag Regex objects in glob and exclude fields
